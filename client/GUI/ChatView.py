...
class ChatView(wx.Frame):
    ...
    def InitUI(self):
        ...
        toolbar.AddLabelTool(ID_SETTINGSTOOL, '', wx.Bitmap('GUI/icons/settings_button.png'))
        toolbar.Realize()
    ...
    def OnUpdateChatView(self, sender, destination, text):
        ...
        print '%s %s %s' % (sender, destination, text)
        if destination == '*':
            destination = ''            
        ...
        colorLen = pos + 10
        self.viewctrl.SetStyle(pos, colorLen,  wx.TextAttr("blue"))
        pos = colorLen + 1
        colorLen = pos + len(sender)
        self.viewctrl.SetStyle(pos, colorLen,  wx.TextAttr("blue"))
        pos = colorLen + 1
        colorLen = pos + len(destination)
        self.viewctrl.SetStyle(pos, colorLen,  wx.TextAttr("red"))
